...
xui_repo="${XUI_REPO:=gzjacktang/3x-ui}"
xui_github="https://github.com/${xui_repo}"
xui_raw="https://raw.githubusercontent.com/${xui_repo}"
...
install_x-ui() {
    ...
    # Download resources
    if [ $# == 0 ]; then
        # This fork publishes main as a rolling pre-release instead of GitHub's
        # stable "latest" release. Keep that implementation detail out of the
        # normal installation command.
        tag_version="dev-latest"
        echo -e "Installing the latest build from this repository..."
        curl -fLR --retry 5 --retry-delay 3 --connect-timeout 15 --speed-limit 1 --speed-time 300 -o ${xui_folder}-linux-$(arch).tar.gz "${xui_github}/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz"
        if [[ $? -ne 0 ]]; then
        ...
        url="${xui_github}/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz"
    ...
    curl -fLRo "${xui_script_temp}" "${xui_raw}/main/x-ui.sh"
    if [[ $? -ne 0 ]]; then
        rm -f "${xui_script_temp}"
    ...
    if [[ $release == "alpine" ]]; then
        ...
        rm -f "${xui_rc_temp}"
        curl -fLRo "${xui_rc_temp}" "${xui_raw}/main/x-ui.rc"
        if [[ $? -ne 0 ]]; then
            rm -f "${xui_rc_temp}"
        ...
        if [ "$service_installed" = false ]; then
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    service_unit_url="${xui_raw}/main/x-ui.service.debian"
                    ;;
                arch | manjaro | parch)
                    service_unit_url="${xui_raw}/main/x-ui.service.arch"
                    ;;
                *)
                    service_unit_url="${xui_raw}/main/x-ui.service.rhel"
                    ;;
            esac
