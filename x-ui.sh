...
install() {
    bash <(curl -Ls "${xui_raw}/main/install.sh")
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls "${xui_raw}/main/update.sh")
    if [[ $? == 0 ]]; then
...
update_dev() {
    ...
    XUI_UPDATE_TAG="dev-latest" bash <(curl -Ls "${xui_raw}/main/update.sh")
    if [[ $? == 0 ]]; then
...
update_menu() {
    ...
    if replace_xui_script "${xui_raw}/main/x-ui.sh" "false"; then
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls \"${xui_raw}/v$tag_version/install.sh\") v$tag_version"
...
uninstall() {
    ...
    echo -e "${green}bash <(curl -Ls ${xui_raw}/main/install.sh)${plain}"
    echo ""
...
update_shell() {
    if replace_xui_script "https://github.com/${xui_repo}/raw/main/x-ui.sh" "true"; then
